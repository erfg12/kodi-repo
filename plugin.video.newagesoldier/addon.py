...
dataroot = xbmc.translatePath(
    'special://profile/addon_data/%s' % __addonname__).decode('utf-8')
...
if mode is None:
    ...
    params = {'q': 'NewAge Soldier', 'key': 'AIzaSyC7YrCyuR9fccM3aSpGcfZhAvVynfyc53k',
    ...
    for i in data['items']:
        ...
        li = xbmcgui.ListItem(
            i['snippet']['title'], iconImage=i['snippet']['thumbnails']['default']['url'])
        xbmcplugin.addDirectoryItem(
            handle=addon_handle, url=url, listitem=li, isFolder=True)
        break

    params = {'q': 'goonnoodles', 'key': 'AIzaSyC7YrCyuR9fccM3aSpGcfZhAvVynfyc53k',
              'part': 'snippet', 'type': 'channel'}
    url = '%s?%s' % (searchUrl, urllib.urlencode(params))
    response = urllib.urlopen(url).read()
    data = json.loads(response)
    ...
    params = {'key': 'AIzaSyC7YrCyuR9fccM3aSpGcfZhAvVynfyc53k',
              'part': 'snippet', 'channelId': args['chanID'][0]}
    ...
    params = {'key': 'AIzaSyC7YrCyuR9fccM3aSpGcfZhAvVynfyc53k',
              'playlistId': args['plID'][0], 'part': 'snippet', 'maxResults': 50}
